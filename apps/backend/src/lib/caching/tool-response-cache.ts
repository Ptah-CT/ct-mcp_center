...
export class ToolResponseCache {
  ...
  static async create(
    ...
    const instance = new ToolResponseCache(maxMemoryEntries, defaultTtlSeconds, cleanupIntervalMs, false);
  ...
  private async initializeRedis(): Promise<void> {
    ...
    try {
      ...
      if (process.env.NODE_ENV !== 'production') {
        throw new Error(`FAIL FAST: ${errorMessage}. Fix Redis configuration or remove Redis environment variables to use memory-only cache.`);
      }
      
      // In production: log extensively but don't crash
      logger.error("Production fallback: continuing with memory-only cache", error);
      this.redis = null;
    }
  }
  ...
  private generateCacheKey(key: ToolCacheKey): string {
    const paramStr = JSON.stringify(key.parameters, Object.keys(key.parameters).sort());
    const keyParts = [
      key.serverUuid,
      key.toolName,
   ...
   * Cache tool response
   */
  async set(key: ToolCacheKey, response: any, ttlSeconds?: number): Promise<void> {
    const cacheKey = this.generateCacheKey(key);
    ...
    if (this.redis && ttl > 60) { // Only cache in Redis for longer TTLs
      try {
  ...
  async reconnectRedis(): Promise<boolean> {
    ...
    try {
      ...
      logger.info("Redis reconnection successful", { redisEnabled: this.redis !== null });
  ...
  async getStatus(): Promise<CacheStatus> {
    ...
    const memoryUsageMB = Math.round(memoryUsageBytes / (1024 * 1024) * 100) / 100;
