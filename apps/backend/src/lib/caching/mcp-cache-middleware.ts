...
import { toolResponseCache, ToolCacheKey } from "./tool-response-cache";
...
export class McpCacheMiddleware {
   ...
   */
  private isCacheableToolCall(message: JSONRPCMessage): message is JSONRPCRequest {
    return (
      message &&
      ...
      async send(message: JSONRPCMessage): Promise<void> {
        ...
        if (this.isCacheableToolCall(message)) {
          ...
          try {
            ...
            logger.error("Cache check failed, proceeding without cache", error, {
              toolName: cacheKey.toolName,
              serverUuid: cacheKey.serverUuid
            });
          }
