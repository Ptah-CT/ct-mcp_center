import { configure, debug, info, warning, error, trace } from 'logfire';
...
export const logger = {
  ...
  mcp: {
    toolsFiltered: (originalCount: number, filteredCount: number, namespace: string, details?: any) => {
    ...
    serverConnection: (serverName: string, status: 'success' | 'failed', details?: any) => {
      const message = `MCP Server ${serverName} connection ${status}`;
      if (status === 'success') {
        console.log(`[MCP-CONNECTION] ${message}`, details || '');
        info('mcp.server.connection', { server_name: serverName, status, details });
      } else {
        console.error(`[MCP-CONNECTION] ${message}`, details || '');
        error('mcp.server.connection_failed', { server_name: serverName, status, details });
      }
    },

    sessionCreated: (sessionId: string, namespace: string, serverCount: number) => {
      ...
      info('mcp.session.created', { session_id: sessionId, namespace, server_count: serverCount });
    }
  }
};
