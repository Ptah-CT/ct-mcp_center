...
// Cleanup function for Streamable HTTP sessions
...
// API-Key cleanup function with optional session targeting
const cleanupApiKey = async (apiKey: string, sessionId?: string) => {
  if (sessionId) {
    ...
    // Clean up both SSE-style and Streamable sessions with the same sessionId if present
    await cleanupSession(sessionId);
    await cleanupStreamableSession(sessionId).catch(() => {});
    return;
  }
  ...
  if (sessions && sessions.size > 0) {
    ...
    console.log(`No active MetaMCP SSE sessions to clean up for API-Key ${apiKey}.`);
  }

  // Clean up all streamable sessions for this API key
  const sessionsForApiKey = Array.from(streamableHttpSessions.entries()).filter(
    ([, session]) => session.apiKey === apiKey,
  );
  for (const [sid] of sessionsForApiKey) {
    await cleanupStreamableSession(sid);
  }
};

// Update last access time for sessions
...
const performTimeBasedCleanup = async () => {
  ...
  const sseSessionsToCleanup: { apiKey: string; sessionId: string }[] = [];
  const streamableSessionsToCleanup: string[] = [];

  // Check all tracked SSE sessions for idle timeout
  for (const [sessionId, lastAccess] of transportLastAccess.entries()) {
    ...
    if (idleTime > MAX_IDLE_TIME) {
      ...
      if (apiKey) {
        sseSessionsToCleanup.push({ apiKey, sessionId });
      } else {
        console.warn(
  ...
  // Check Streamable HTTP sessions for idle timeout
  for (const [sessionId, lastAccess] of streamableSessionLastAccess.entries()) {
    const idleTime = now.getTime() - lastAccess.getTime();
    if (idleTime > MAX_IDLE_TIME) {
      streamableSessionsToCleanup.push(sessionId);
    }
  }

  // Cleanup idle SSE sessions
  for (const { apiKey, sessionId } of sseSessionsToCleanup) {
    console.log(
  ...
  // Cleanup idle Streamable HTTP sessions
  for (const sessionId of streamableSessionsToCleanup) {
    console.log(
  ...
  console.log(
    `MetaMCP time-based cleanup completed. Cleaned up ${sseSessionsToCleanup.length} idle SSE transports and ${streamableSessionsToCleanup.length} idle Streamable HTTP sessions.`,
  );
};
